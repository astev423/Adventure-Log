# Stretch
-Let users upload a profile photo
-Let users make their reviews private or public
-Hide reviews which hides them in both maps and reviews page
-Keep list of hidden reviews, let users unhide them if they want
-Sort by saved reviews
-Add ability to sort from reviews highest to lowest
-Let users change account email and username

# Refactor
-Make wrapper function which produces basic layout for the different review page filters
and I can just pass my unique logic (child) in as a parameter
